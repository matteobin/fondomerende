...
?>
    <h2><?php echo(ucfirst(getTranslatedString('main', 9))); ?></h2>
</header>
<?php if (isset($response['response']['message'])): ?> 
        <p><?php echo($response['response']['message']); ?></p>
<?php endif; ?>
<h3><?php echo(ucfirst(getTranslatedString('main', 2))); ?>: <?php if (isset($_POST['user-funds-amount'])) {echo($_POST['user-funds-amount']);} else {echo($response['data']['user-funds-amount']);} ?> €</h3>
<form action="<?php echo(BASE_DIR); ?>index.php?view=deposit" method="POST">
    ...
    <label for="deposit-amount-input"><?php echo(ucfirst(getTranslatedString('deposit', 1))); ?></label>
    ...
    <input type="submit" value="<?php echo(ucfirst(getTranslatedString('main', 9))); ?>">
</form>
<script>
    function askDepositConfirm(event) {
        event.preventDefault();
        if (confirm('<?php echo(ucfirst(getTranslatedString('main', 9))) ?> '+event.target[2].value+' €?')) {
            event.target.submit();
        }
    }
