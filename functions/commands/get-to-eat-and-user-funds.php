...
function getToEatAndUserFunds(DbManager $dbManager, $userId) {
    ...
    while ($snacksStockRow = $dbManager->result->fetch_assoc()) {
		$expiration = new DateTime($snacksStockRow['expiration']);
		$expiration = $expiration->format(getFormat(3));
        array_push($snacks, array('id'=>$snacksStockRow['snack_id'], 'friendly-name'=>$snacksStockRow['friendly_name'], 'quantity'=>$snacksStockRow['quantity'], 'expiration'=>$expiration));
    }
