...
function eat($userId, $snackId, $quantity) {
    global $dbManager;
    $dbManager->query('SELECT outflow_id, price_per_snack FROM crates WHERE snack_id=? AND snack_quantity!=? ORDER BY expiration ASC LIMIT 1', [$snackId, 0], 'ii');
    ...
    if (isset($outflowId)) {
        ...
        $dbManager->query('INSERT INTO actions (user_id, command_id, snack_id, snack_quantity, funds_amount, outflow_id) VALUES (?, ?, ?, ?, ?, ?)', [$userId, 8, $snackId, $quantity, $totalPrice, $outflowId], 'iiiidi');
        return ['success'=>true, 'status'=>200];
    } else {
        return ['success'=>false, 'status'=>404, 'message'=>getTranslatedString('response-messages', 31).$snackId.'.'];
    }
}
