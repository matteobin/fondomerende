...
function deleteExpiredTokens($fromCli=true, $verbose=0) {
    ...
    if ($verbose) {
        ...
        $dbManager->query($query, [$nowToday], 's');
        $toDelete = [];
        ...
        for ($i=0; $i<$toDeleteNum; $i++) {
            $dbManager->query('DELETE FROM tokens WHERE tokens.id=?', [$toDelete[$i]['id']], 'i');
            if ($fromCli) {
                if (!$i && $verbose==2) {
                    echo "\n";
                }
                ...
                $response['data']['deleted-tokens'][$i] = ['token'=>$toDelete[$i]['token'], 'user-id'=>$toDelete[$i]['id']];
                if ($verbose==2) {
        ...
        $dbManager->query('DELETE FROM tokens WHERE tokens.expires_at IS NOT NULL AND tokens.expires_at<=?', [$nowToday], 's');
    }
    if ($fromCli) {
        return true;
    } else {
...
if (php_sapi_name()=='cli') { 
    ...
    if (MAINTENANCE) {
        ...
        try {
            ...
            $lockTables = $verbose ? ['tokens'=>'w', 'users'=>'r'] : ['tokens'=>'w'];
